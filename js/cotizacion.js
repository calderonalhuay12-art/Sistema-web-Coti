...
// Clave específica para conservar actividades por Magnitud + Equipo.
function obtenerClaveActividad() {
    const magnitud = document.getElementById('item-magnitud').value.trim();
    const equipo = document.getElementById('item-equipo').value.trim();
    return (magnitud && equipo) ? `${magnitud}::${equipo}` : 'General';
}

function obtenerActividadesCatalogo(clave) {
    const actividadesEspecificas = catalogoActividades[clave] || [];
    // Los catálogos previos se guardaban bajo "General". Se muestran como
    // respaldo solo mientras la combinación nueva todavía no tenga actividades.
    const actividadesLegacy = clave !== 'General' && actividadesEspecificas.length === 0
        ? (catalogoActividades.General || [])
        : [];
    return [...new Set([...actividadesEspecificas, ...actividadesLegacy])];
}
...
function abrirModalActividades() {
    ...
    const todasLasActividades = [...new Set([...obtenerActividadesCatalogo(clave), ...Object.keys(marcadas)])];
...
function renderTables() {
    ...
    uiBody.innerHTML = '';
    printBody.innerHTML = '';
    const totales = calcularTotalesCotizacion(itemsData, {
        incluidos: document.getElementById('check-viaticos').checked,
        monto: document.getElementById('viaticos-monto').value,
        descuento: document.getElementById('viaticos-dscto').value,
    });

    itemsData.forEach((row, index) => {
        const itemNormalizado = totales.itemsNormalizados[index];
        const subtotalBruto = itemNormalizado.valorUnitario * itemNormalizado.cantidad;
        const montoDsctoTotal = itemNormalizado.descuentoMonto * itemNormalizado.cantidad;
    ...
    if (totales.aplicarViaticos) {
        ...
        printBody.innerHTML += `<tr><td class="equipo">${viaticosIndex}</td><td class="desc-cell">${descHTML}</td><td class="tipo">No aplica</td><td class="total">No aplica</td><td>Campo (in situ)</td><td style="font-weight:bold;">NO APLICA</td><td>S/ ${formatMoney(totales.descuentoViaticos)}</td><td>S/ ${formatMoney(totales.montoViaticos)}</td><td>1</td><td class="total">S/ ${formatMoney(totales.viaticosNetos)}</td></tr>`;
    }

    document.getElementById('lbl-subtotal').innerText = "S/ " + formatMoney(totales.subtotalNeto);
    document.getElementById('lbl-igv').innerText = "S/ " + formatMoney(totales.igv);
    document.getElementById('lbl-total').innerText = "S/ " + formatMoney(totales.total);
    document.getElementById('t-descuento').innerText = "S/. " + formatMoney(totales.descuentoTotal);
    document.getElementById('t-subtotal').innerText = formatMoney(totales.subtotalNeto);
    document.getElementById('t-igv').innerText = formatMoney(totales.igv);
    document.getElementById('t-total').innerText = formatMoney(totales.total);
    document.getElementById('out-monto-letras').innerText = "Son: " + numeroALetras(totales.total);
}
