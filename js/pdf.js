...
gy += 3.5;
    ...
    const filas = totales.itemsNormalizados.map((it, idx) => {
        const bruto = it.valorUnitario * it.cantidad;
        const dscto = it.descuentoMonto * it.cantidad;
    ...
    if (totales.aplicarViaticos) {
        ...
        filas.push([filas.length + 1, descViat, 'No aplica', 'No aplica', 'Campo', 'NO APLICA',
                    'S/ ' + formatMoney(totales.descuentoViaticos), 'S/ ' + formatMoney(totales.montoViaticos), 1, 'S/ ' + formatMoney(totales.viaticosNetos)]);
    }
    ...
    doc.text('Descuento:', 165, fy, {align:'right'}); doc.text(`S/. ${formatMoney(totales.descuentoTotal)}`, 200, fy, {align:'right'}); fy += 4;
    doc.text('Sub total:', 165, fy, {align:'right'}); doc.text(`${formatMoney(totales.subtotalNeto)}`, 200, fy, {align:'right'}); fy += 4;
    doc.text('IGV (18%):', 165, fy, {align:'right'}); doc.text(`${formatMoney(totales.igv)}`, 200, fy, {align:'right'}); fy += 5;
    doc.setFont(undefined, 'bold'); doc.setFontSize(8.5);
    doc.text('Total:', 165, fy, {align:'right'}); doc.text(`${formatMoney(totales.total)}`, 200, fy, {align:'right'});
    fy += 6;

    doc.setFont(undefined, 'bolditalic'); doc.setFontSize(7.5);
    doc.text(`Son: ${numeroALetras(totales.total)}`, 200, fy, { align: 'right' });
